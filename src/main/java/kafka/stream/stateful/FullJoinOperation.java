...
public class FullJoinOperation {
	...
	public static void main(String[] args) {
        ...
        userKS.outerJoin(addressKS, 
        		(left, right) -> left + "----" + right, JoinWindows.of(Duration.ofMinutes(1)),
                StreamJoined.with(Serdes.String(), Serdes.String(), Serdes.String()))
        .print(Printed.<String, String>toSysOut().withLabel("full-join"));
        
        KafkaHelper.start(new KafkaStreams(builder.build(), KafkaHelper.config(KafkaHelper.STATEFUL_FULL_JOIN_APP_ID)));
	}
}
