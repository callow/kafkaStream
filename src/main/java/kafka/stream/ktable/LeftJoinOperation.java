...
public class LeftJoinOperation {
	...
	public static void main(String[] args) {
        ...
        // Stream Left Join Table， 如果Table中有值才会往下走。
        KStream<String, String> result = stream
        		.leftJoin(table, (userName, address) -> userName + " from " +  address == null ? "N/A" : address, 
        				Joined.with(Serdes.String(), Serdes.String(), Serdes.String()));

        result.print(Printed.<String, String>toSysOut().withLabel("kstream-inner-join-ktable"));
        
        KafkaHelper.start(new KafkaStreams(builder.build(), KafkaHelper.config(KafkaHelper.KTABLE_LEFT_JOIN_APP_ID)));
	}
}
